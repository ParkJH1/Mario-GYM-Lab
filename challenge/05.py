...
class MarioTileMap(QWidget):
    ...
    def paintEvent(self, e):
        ...
        try:
            ...
            for i in range(full_screen_tiles.shape[0]):
                for j in range(full_screen_tiles.shape[1]):
                    if full_screen_tiles[i][j] > 0:
                        full_screen_tiles[i][j] = 1
                    if full_screen_tiles[i][j] == -1:
                        full_screen_tiles[i][j] = 2
                        painter.setBrush(QBrush(Qt.GlobalColor.red))
                    else:
                        painter.setBrush(QBrush(QColor.fromHslF(125 / 239, 0 if full_screen_tiles[i][j] == 0 else 1, 120 / 240)))
            ...
            px = ((screen_x_position_offset + player_x_position_current_screen_offset + 8) % 512) // 16
